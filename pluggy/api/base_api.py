...
class BaseApi:
    ...
    def __init__(self, client_id: str, client_secret: str, session: httpx.AsyncClient, base_url: str = None, api_key: Optional[str] = None):
    ...
    async def create_get_request(self, endpoint: str, params: Optional[QueryParameters] = None) -> PageResponse:
    ...
    async def create_post_request(self, endpoint: str, params: Optional[QueryParameters], body: Optional[Dict[str, object]]):
        return  await self.create_mutation_request(endpoint=endpoint, params=params, body=body, method='POST')

    async def create_put_request(self, endpoint: str, params: Optional[QueryParameters], body: Optional[Dict[str, object]]):
        return  await self.create_mutation_request(endpoint, params, body, method='PUT')

    async def create_patch_request(self, endpoint: str, params: Optional[QueryParameters] = None, body: Optional[Dict[str, object]] = None):
        return  await self.create_mutation_request('PATCH', endpoint, params, body)

    async def create_delete_request(self, endpoint: str, params: Optional[QueryParameters] = None, body: Optional[Dict[str, object]] = None):
        return  await self.create_mutation_request('DELETE',endpoint, params, body)

    async def create_mutation_request(self, method: str, endpoint: str, params: Optional[Dict[str, Any]] = None, body: Optional[Dict[str, Any]] = None) -> Any:
        api_key = await self.get_api_key()
        url = f"{self.base_url}/{endpoint}{self.map_to_query_string(params)}"
